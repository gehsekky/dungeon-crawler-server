...
class Game {
  ...
  initialize() {
    ...
    return partyMember.initAttributes()
    .then(() => {
  ...
  static load(dbGame) {
    ...
    game.room = new Room(dbGame.Rooms[0].locationX, dbGame.Rooms[0].locationY, dbGame.Rooms[0].RoomType.name);
    game.room.gameId = dbGame.gameId;
    game.room.roomId = parseInt(dbGame.Rooms[0].roomId);
    game.room.roomTypeId = parseInt(dbGame.Rooms[0].RoomType.roomTypeId);
    ...
    return Promise.all(dbGame.PartyMembers.map((dbPartyMember) => PartyMember.load(dbPartyMember)))
    .then((partyMembers) => {
      game.party = partyMembers;
      return game
    })
    .catch((err) => {
      console.error(err);
    ...
    if (!this.gameId) {
      ...
      return db.sequelize.transaction((t) => {
        ...
        .then((room) => {
          ...
          console.log('about to save party members', this.PartyMembers);
